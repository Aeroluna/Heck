...
{
    private static readonly MethodInfo _beatEffectInit = AccessTools.Method(
        typeof(BeatEffect),
        nameof(BeatEffect.Init));

    private static readonly MethodInfo _jumpStartPosGetter = AccessTools.PropertyGetter(
        typeof(NoteController),
        nameof(NoteController.jumpStartPos));

    private static readonly MethodInfo _positionGetter = AccessTools.PropertyGetter(
        typeof(Transform),
        nameof(Transform.position));

    private static readonly MethodInfo _rotationGetter = AccessTools.PropertyGetter(
        typeof(Transform),
        nameof(Transform.rotation));

    private static readonly MethodInfo _transformGetter = AccessTools.PropertyGetter(
        typeof(Component),
