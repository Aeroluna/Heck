...
internal class NoteJumpNoodlifier : IAffinity, IDisposable
{
    ...
    private static readonly FieldInfo _endRotationField = AccessTools.Field(
        typeof(NoteJump),
        nameof(NoteJump._endRotation));

    private static readonly MethodInfo _getTransform = AccessTools.PropertyGetter(
        typeof(Component),
        nameof(Component.transform));

    private static readonly FieldInfo _inverseWorldRotationField =
        AccessTools.Field(typeof(NoteJump), nameof(NoteJump._inverseWorldRotation));

    private static readonly FieldInfo _jumpDurationField = AccessTools.Field(
        typeof(NoteJump),
        nameof(NoteJump._jumpDuration));

    private static readonly FieldInfo _localPositionField = AccessTools.Field(
        typeof(NoteJump),
        nameof(NoteJump._localPosition));

    private static readonly MethodInfo _localRotationSetter =
        AccessTools.PropertySetter(typeof(Transform), nameof(Transform.localRotation));

    private static readonly FieldInfo _middleRotationField = AccessTools.Field(
        typeof(NoteJump),
        nameof(NoteJump._middleRotation));

    private static readonly FieldInfo _missedTimeField = AccessTools.Field(
        typeof(NoteJump),
        nameof(NoteJump._missedTime));

    private static readonly FieldInfo _playerTransformsField = AccessTools.Field(
        typeof(NoteJump),
        nameof(NoteJump._playerTransforms));

    private static readonly FieldInfo _rotatedObjectField = AccessTools.Field(
        typeof(NoteJump),
        nameof(NoteJump._rotatedObject));

    private static readonly FieldInfo _rotateTowardsPlayerField =
        AccessTools.Field(typeof(NoteJump), nameof(NoteJump._rotateTowardsPlayer));

    private static readonly FieldInfo _startRotationField = AccessTools.Field(
        typeof(NoteJump),
        nameof(NoteJump._startRotation));

    private static readonly FieldInfo _threeQuartersMarkReportedField = AccessTools.Field(
        typeof(NoteJump),
        nameof(NoteJump._threeQuartersMarkReported));

    private static readonly MethodInfo _noteMissedTimeAdjust = AccessTools.Method(
        ...
        _doNoteLook =
            InstanceTranspilers
                .EmitInstanceDelegate<
                    Action<float, Quaternion, Quaternion, Quaternion, PlayerTransforms, Transform, Transform,
                        Quaternion, bool>>(DoNoteLook);
        ...
        bool rotateTowardsPlayer)
    {
        ...
        Quaternion a = num2 < 0.125f
            ...
            : Quaternion.Slerp(
                ...
                Mathf.Sin((num2 - 0.125f) * Mathf.PI * 2f));
