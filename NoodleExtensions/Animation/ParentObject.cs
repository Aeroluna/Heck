...
internal class ParentObject : MonoBehaviour
{
    ...
    internal HashSet<Track> ChildrenTracks { get; } = [];
    ...
    private void Update()
    {
        ...
        Vector3 positionVector = worldRotationQuatnerion *
        ...
        if (rotation.HasValue || position.HasValue)
        {
            ...
            positionVector = worldRotationQuatnerion *
                             ((positionOffset + _startPos) * StaticBeatmapObjectSpawnMovementData.kNoteLinesDistance);
        }
...
internal class AssignTrackParent : ICustomEvent
{
    ...
    private readonly HashSet<ParentObject> _parentObjects = [];
