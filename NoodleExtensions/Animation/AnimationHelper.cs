        ...
        out Vector3? definitePosition)
    {
        ...
        if (!pathDefinitePosition.HasValue && tracks != null)
        {
            if (tracks.Count > 1)
            {
                ...
                {
                    sumPathDefinitePosition = SumVectorNullables(
                        sumPathDefinitePosition,
                        track.GetVector3PathProperty(DEFINITE_POSITION, time));
                }
        ...
        if (pathDefinitePosition.HasValue)
        {
            ...
            if (tracks != null)
            {
                if (tracks.Count > 1)
                {
                    ...
                    foreach (Track track in tracks)
                    {
                        if (!hasPathPosition)
                        {
                            sumPathPosition = SumVectorNullables(
                                sumPathPosition,
                                track.GetVector3PathProperty(OFFSET_POSITION, time));
                        }

                        sumPositionOffset = SumVectorNullables(
                            sumPositionOffset,
                            track.GetProperty<Vector3>(OFFSET_POSITION));
                    }
            ...
            definitePosition =
                (SumVectorNullables(positionOffset, pathDefinitePosition) *
                 StaticBeatmapObjectSpawnMovementData.kNoteLinesDistance)?.Mirror(_leftHanded);
        }
        else
        {
        ...
        if (tracks != null)
        {
            ...
            if (tracks.Count > 1)
            {
                ...
                foreach (Track? track in tracks)
                {
                    ...
                    if (!hasPathRotation)
                    {
                        multPathRotation = MultQuaternionNullables(
                            multPathRotation,
                            track.GetQuaternionPathProperty(OFFSET_ROTATION, time));
                    }
                    ...
                    {
                        multPathLocalRotation = MultQuaternionNullables(
                            multPathLocalRotation,
                            track.GetQuaternionPathProperty(LOCAL_ROTATION, time));
                    }

                    if (!hasPathDissolve)
                    {
                        multPathDissolve = MultFloatNullables(
                            multPathDissolve,
                            track.GetLinearPathProperty(DISSOLVE, time));
                    }

                    if (!hasPathDissolveArrow)
                    {
                        multPathDissolveArrow = MultFloatNullables(
                            multPathDissolveArrow,
                            track.GetLinearPathProperty(DISSOLVE_ARROW, time));
                    }

                    if (!hasPathCuttable)
                    {
                        multPathCuttable = MultFloatNullables(
                            multPathCuttable,
                            track.GetLinearPathProperty(INTERACTABLE, time));
                    }

                    sumTrackPosition = SumVectorNullables(
                        sumTrackPosition,
                        track.GetProperty<Vector3>(OFFSET_POSITION));
                    multTrackRotation = MultQuaternionNullables(
                        multTrackRotation,
                        track.GetProperty<Quaternion>(OFFSET_ROTATION));
                    multTrackScale = MultVectorNullables(multTrackScale, track.GetProperty<Vector3>(SCALE));
                    multTrackLocalRotation = MultQuaternionNullables(
                        multTrackLocalRotation,
                    ...
                    multTrackDissolveArrow = MultFloatNullables(
                        multTrackDissolveArrow,
                        track.GetProperty<float>(DISSOLVE_ARROW));
            ...
            positionOffset = SumVectorNullables(trackPosition, pathPosition) *
