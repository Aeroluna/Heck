...
internal class ChromaAnimateComponentData : ICustomEventCustomData
{
        ...
        Dictionary<string, List<object>> pointDefinitions)
    {
        ...
        List<KeyValuePair<string, object?>> componentKeys =
            customData.Where(n => availableNames.Contains(n.Key)).ToList();
        List<(string ComponentName, Dictionary<string, PointDefinition<float>?> PointDefinition)> coroutineInfos = [];
        foreach ((string key, object? value) in componentKeys)
        {
            ...
            Dictionary<string, PointDefinition<float>?> componentPoints = component.Keys
                .ToDictionary(
                    ...
                    propertyKey => component.GetPointData<float>(propertyKey, pointDefinitions));
    ...
    internal IReadOnlyList<(string ComponentName, Dictionary<string, PointDefinition<float>?> PointDefinition)>
        CoroutineInfos { get; }
        ...
        bool v2)
    {
        ...
        if (lightID != null)
        {
            LightID = lightID switch
            {
                ...
                long lightIDint => [(int)lightIDint],
                _ => null
            };
        }
    }
