...
internal class RingStepChromafier : IAffinity, IDisposable
{
    ...
    private RingStepChromafier([Inject(Id = ChromaController.ID)] DeserializedData deserializedData)
    {
        ...
        _getPrecisionStep =
            InstanceTranspilers.EmitInstanceDelegate<Func<float, BasicBeatmapEventData, float>>(GetPrecisionStep);
        _getPrecisionSpeed =
            InstanceTranspilers.EmitInstanceDelegate<Func<float, BasicBeatmapEventData, float>>(GetPrecisionSpeed);
    }
