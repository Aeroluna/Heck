...
internal static class LegacyEnvironmentRemoval
{
    ...
    {
        IEnumerable<string>? objectsToKill =
            customBeatmap.beatmapCustomData.Get<List<object>>(V2_ENVIRONMENT_REMOVAL)?.Cast<string>();
        ...
        Plugin.Log.Warn(
            "Please do not use Legacy Environment Removal for new maps as it is deprecated and its functionality in future versions of Chroma cannot be guaranteed");
        ...
        foreach (string s in objectsToKill)
        {
            ...
            else
            {
                foreach (GameObject n in gameObjects
                             .Where(
                                 obj => obj.name.Contains(s) &&
                                        (obj.scene.name?.Contains("Environment") ?? false) &&
                                        (!obj.scene.name?.Contains("Menu") ?? false)))
                {
                    n.SetActive(false);
                }
            }
