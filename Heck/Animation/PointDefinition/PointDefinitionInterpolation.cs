        ...
        where T : struct
    {
        ...
        public static PointDefinitionInterpolation<T> CreateDerived()
        {
            PointDefinitionInterpolation<T>? result = typeof(T) switch
            {
                ...
                var n when n == typeof(Vector3) => new Vector3PointDefinitionInterpolation() as PointDefinitionInterpolation<T>,
                var n when n == typeof(Vector4) => new Vector4PointDefinitionInterpolation() as PointDefinitionInterpolation<T>,
                var n when n == typeof(Quaternion) => new QuaternionPointDefinitionInterpolation() as PointDefinitionInterpolation<T>,
        ...
        internal T? Interpolate(float time)
        {
            ...
            return _previousPointData == null ? _basePointData.Interpolate(time) : InterpolatePoints(_previousPointData, _basePointData, Time, time);
        }

        protected abstract T InterpolatePoints(PointDefinition<T> previousPoint, PointDefinition<T> basePoint, float interpolation, float time);
    }
}
