...
namespace Heck.Animation
{
    ...
    public abstract class PointDefinition<T> : IPointDefinition
        where T : struct
    {
        private readonly List<IPointData> _points = new();
        ...
        {
            return "{" + string.Join(", ", _points.Select(n =>
            {
                string result = n.ToString();
                string added = $", {n.Time}" + (n.Easing != Functions.easeLinear ? ", " + n.Easing : string.Empty);
